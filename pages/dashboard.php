...
$q2b = $pdo->prepare("SELECT COUNT(*) FROM pantry_items

    WHERE user_id = ? AND expiry_date < CURDATE()");

$q2b->execute([$user_id]);

$expired_count = $q2b->fetchColumn();
...
// Expiring items alert (within 7 days, not yet expired)
...
$q6 = $pdo->prepare("SELECT pi.*, p.name as product_name,
    ...
    WHERE pi.user_id = ? AND pi.expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY)
...
// Already-expired items alert

$q7 = $pdo->prepare("SELECT pi.*, p.name as product_name,
    ...
    <div class="stat-grid">
      ...
      <div class="stat-card">
        ...
        <div>
          ...
          <div class="stat-val"><?= $expired_count ?></div>
        ...
        <div class="card">
          ...
          <?php foreach ($expired_items as $item): ?>
            ...
            <div class="item-row">
              ...
              <span class="badge badge-danger">Expired <?= abs($item['days_left']) ?>d ago</span>
